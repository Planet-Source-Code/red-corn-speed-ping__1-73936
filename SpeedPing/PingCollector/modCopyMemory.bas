...
'        wParam: handle of sending window
    ...
    Case glMyPingMsg
        ...
        Case MSG_AGENT_SAYGOODBYE 'Say Goodbye時用postmessage,離開的乾脆一點
            AgentID = lParam
        ...
        Select Case cdCopyData.dwData
            ...
            Case MSG_PLS_PINGCOLLECTOR_LOADINI
                ...
                If glPingCount > 0 And _
                    ...
                    glDelayStart > 0 Then
                    frmMain.InitAgentStuff '先初始化Agent Array, pingnode ub & lb...
                    'ReportLoadIniOK
                    Call ReportReady '改成回報自己ready即可,agent是否ready,由manager自己處理
                Else
                    MsgBox "系統參數錯誤!", vbExclamation, MsgTitle
            ...
            Case MSG_PLS_PINGCOLLECTOR_LOADPINGLIST
                ...
                ReplyMessage 1
                frmMain.DoLoadPingList num '--> 將回報LoadList OK / ERR
...
'                ReplyMessage 1
'                'If AgentID > 0 And AgentID < 11 Then '防止錯誤
...
'                'End If
            ...
            Case MSG_REPORT_PINGSTAT
                ...
                If aryAgentPingInfo(6, AgentID) <> 1 Then
                    ReplyMessage 1
                    frmMain.StopPing
                    MsgBox "MSG_REPORT_PINGSTATUS_RTT counter錯誤! ping停止!"
                    Exit Function
                End If
            ...
            Case MSG_REPORT_PINGSTATUS_OK
                ...
                If aryAgentPingInfo(6, AgentID) <> 2 Then
                    ReplyMessage 1
                    frmMain.StopPing
                    MsgBox "MSG_REPORT_PINGSTATUS_OK counter錯誤! ping停止!"
                    Exit Function
                End If
                ...
                If aryAgentPingInfo(4, AgentID) <> SN Then
                    ...
                    Exit Function
                End If
                '新版的已不再判斷pingcount了,因為pingcount = glpingcount時才會收到訊息
            ...
            Case MSG_AGENT_SAYHELLO '會得到agentid & agenthwnd
...
'                MsgBox "aryAgentHwnd(AgentID)=" & aryAgentHwnd(AgentID)
            ...
            Case MSG_AGENT_READY 'Agent Load List完之後會回報Ready
                ReplyMessage 1
                AgentID = wParam
                frmMain.AgentReady AgentID '-->告訴Manager,我也Ready了
        End Select
...
Public Sub ReportReady()
    ...
    cdCopyData.lpData = VarPtr(DoCmd)
    SendMessage glPingManagerHwnd, WM_COPYDATA, glMyHwnd, cdCopyData
    '在ready的時候manager開始檢查alive,故要啟動blink
    frmMain.StartBlink
End Sub
...
Public Sub TellAgentToLoadPingList(lb As Long, ub As Long, AgentHwnd As Long)
    ...
    Dim num As Long
    num = ub - lb + 1
    cdCopyData.dwData = MSG_AGENT_LOADPINGLIST '自定義數據
...
Public Sub AgentSetCycleInterval(AgentHwnd As Long, CycleInterval As Long)
    ...
    cdCopyData.dwData = MSG_AGENT_SET_CYCLEINTERVAL '自定義數據
...
Public Sub SayHello()
    ...
    If glPingManagerHwnd = 0 Then
        Exit Sub
    End If
    cdCopyData.dwData = MSG_PINGCOLLECTOR_SAYHELLO '自定義數據
...
'
'    cdCopyData.dwData = MSG_PINGCOLLECTOR_REPORT_PING_RESULT '自定義數據
'    cdCopyData.cbData = Len(aryReportData(1, 0)) * NumOfPingNode * 8 '傳送長度
...
Public Sub ReportPingResult()
    ...
    cdCopyData.dwData = MSG_PINGCOLLECTOR_REPORT_PING_RESULT '自定義數據
    cdCopyData.cbData = Len(aryPingStatData(1, 0)) * NumOfPingNode * 8 '傳送長度
...
Public Sub SendEventToManager()
    ...
    If EventBufCount = 0 Then
        cdCopyData.dwData = MSG_PINGCOLLECTOR_SENDEVENT_NOEVENT '自定義數據
        cdCopyData.cbData = Len(EventSN) '傳送長度
        cdCopyData.lpData = VarPtr(EventSN)
        SendMessage glPingManagerHwnd, WM_COPYDATA, glMyHwnd, cdCopyData
        Exit Sub
    Else
        LenOfData = Len(aryEventBuf1(1, 1)) * EventBufCount * 2 '二維陣列
        cdCopyData.dwData = MSG_PINGCOLLECTOR_SENDEVENT_BUF1 '自定義數據
        cdCopyData.cbData = LenOfData '傳送長度
        cdCopyData.lpData = VarPtr(aryEventBuf1(1, 1)) '二維陣列 'vb6 column order
        SendMessage glPingManagerHwnd, WM_COPYDATA, EventSN, cdCopyData

        LenOfData = Len(aryEventBuf2(1)) * EventBufCount
        cdCopyData.dwData = MSG_PINGCOLLECTOR_SENDEVENT_BUF2 '自定義數據
        cdCopyData.cbData = LenOfData '傳送長度
