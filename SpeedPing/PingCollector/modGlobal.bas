...
Public Const MAX_PING_NODES As Long = 30000 '最多3萬
...
Public glPingMaxBurst As Long '= 500 '每個interval送出的packet數量
Public glPingCount As Integer '= 5 '每個節點測試的次數
Public glThreshold As Integer '= 5 '掉幾個包為ping lost
...
Public glContinuedFailAsDown As Integer '取消不用,改用glStatisticsCycle(參數照傳過來沒關係)
...
'//關於agent
Public glAgentCount As Integer 'Ping agent's 個數
...
Public arySN() As Long '匯出agent資料時用
...
Public Enum PingResult
    ...
    RESULT_DOWN = 3
End Enum
'用來判斷上一次是Down / Up, 如果由Up --> Down 或 Down --> Up,則記log
Public aryLastUpDown() As PingResult
'用來記錄上一次的result,用在顯示燈號時,如果目前的result和上一個燈號相同則不用重複設定,以致於影響顯示速度
...
'Public aryReportData() As Long '這是要傳給manager的
...
Public Const PING_NOT_YET As Long = -101 '自定義status
Public Const MY_PING_OK As Long = 1 '自定義status
