...
Private Const MSG_AGENT_CLOSE As Long = MSG_MYPING_BASE + 11 '由server發出
    ...
    Case WM_COPYDATA
        ...
        Select Case cdCopyData.dwData
            Case MSG_AGENT_LOADPINGLIST
                '注意Load Ping List時因為要redim round trip time array時會用到glPingCount,所以要先Load INI才行
                LenOfCopyData = cdCopyData.cbData
                NumOfPingNode = LenOfCopyData \ Len(LenOfCopyData) '同樣是long
...
'                    Next
                    frmMain.LoadPingList '-->會執行ReportReady
                Else
                    ReplyMessage 1
                End If
            Case MSG_AGENT_LOAD_INI
                LenOfCopyData = cdCopyData.cbData
                ininum = LenOfCopyData \ Len(LenOfCopyData) '同樣是long
...
'        Exit Sub
'    End If
'    cdCopyData.dwData = MSG_PINGAGENT_KEEPALIVE_REPLY '自定義數據
...
Public Sub SayHello()
    ...
    cdCopyData.dwData = MSG_AGENT_SAYHELLO '自定義數據
...
Public Sub ReportLoadIniOK()
    ...
    cdCopyData.cbData = Len(DoCmd)
    cdCopyData.lpData = VarPtr(DoCmd)
    '只需向Collector回報 --> Collector會告訴我 --> MSG_AGENT_LOADPINGLIST
    SendMessage glPingCollectorHwnd, WM_COPYDATA, MyAgentID, cdCopyData
End Sub

Public Sub AgentLedBlink(BlinkSwitch As Integer)
    '只向Manager Blink,免得影響Collector的performance
...
Public Sub ReportPingStatus(StartNode As Long, EndNode As Long)
    ...
    cdCopyData.dwData = MSG_REPORT_PINGSTATUS_INFO '自定義數據
    ...
    cdCopyData.cbData = Len(PingInfo(1)) * 5 '傳送長度
    ...
    LenOfData = Len(LenOfData) * Num * 6 '皆為long
    cdCopyData.dwData = MSG_REPORT_PINGSTAT '自定義數據
    cdCopyData.cbData = LenOfData '傳送長度
    cdCopyData.lpData = VarPtr(aryAgentPingResultData(1, StartNode)) '二維陣列 'vb6 column order
    SendMessage glPingCollectorHwnd, WM_COPYDATA, MyAgentID, cdCopyData
    
    cdCopyData.dwData = MSG_REPORT_PINGSTATUS_OK '自定義數據
