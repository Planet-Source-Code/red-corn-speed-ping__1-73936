...
Private Sub cmdClearLog_Click()
    ...
    msg = "您確定要清除超過 " & LogKeepDays & " 天的事件記錄資料嗎?"   ' 定義訊息
    ...
    Set cnn1 = New Connection '要用connection,否則command在第二次執行時會出現以下的清除資料失敗的奇怪現象
    ...
    Exit Sub
ErrHandler:
    MsgBox "執行事件記錄清除時發生錯誤:" & vbCrLf & Err.Description, vbExclamation, MsgTitle
...
Public Sub SetShowNodeName(Optional b As Boolean = True)
    ...
    If ShowNodeName Then
        ...
        lvLog2.Visible = True 'lvLog2有顯示:SN, Nodename, IP addr, Description 1,2,3
    Else
        lvLog1.Visible = True
...
Private Sub Form_Resize()
    ...
    Else
        Set lvlog = lvLog1
        Picture1.Height = 420 '隱藏清除log的功能
...
'            End If
        ...
        If .RecordCount = 0 Then
            ...
            statusbar.Panels(1).Text = "查無資料!"
        Else
            statusbar.Panels(1).Text = "共" & .RecordCount & "筆記錄!"
            .MoveFirst
            x = SendMessage(lvlog.hWnd, WM_SETREDRAW, 0, 0)
            lvlog.ListItems.Clear
...
ErrHandler:
    MsgBox "讀取資料時產生下列錯誤:" & vbCrLf & Err.Description, vbExclamation, MsgTitle
End Sub
...
'lvLog2是所有node的log,所以有double click,可以再叫出個別node的明細
    Dim itemx As ListItem
