...
'新版的流程是先載入PingCollector,然後等PingCollector SayHello
    '先關掉舊的程式
    ...
    For idx = 0 To glAgentCount - 1
        picAgent(idx).Picture = picLed(0).Picture '灰色
        picAgent(idx).Refresh
    Next
    picPingCollector.Picture = picLed(0).Picture '灰色
...
Private Sub cmdInitPing_Click()
    ...
    Call InitAgentLedToGrayColor '全部led設成灰色
    Call ResizeForm
    picPingCollector.Picture = picLed(0).Picture '先設成灰色
    picPingCollector.Refresh
    
    '在create agent之前讀取參數,並一併傳給agent
    ...
    '請PingCollector先載入INI (並初始化Agent Array)
    
    LoadOK = CreatePingCollector '會傳回訊息,故必須要先執行Hook
    If LoadOK Then
...
''    '因為pingagent起來後會向collector say hello!, 而collector必須先讀進ini值當中的值,例如glPingCount,才能初始化
''    '陣列, 在收到
''    Call TellCollectorToLoadINI ' --> PingCollector直接回報Ready--> PingCollectorReady --> createpingagent
...
Private Function CreatePingAgent() As Boolean
    ...
    If Not fs.FileExists(PingAgentPath) Then
        MsgBox "找不到PingAgent程式! (" & PingAgentPath & ")", vbExclamation, MsgTitle
    ...
    For AgentID = 1 To glAgentCount
        ...
        If ret = 0 Then
            MsgBox "執行CreatePingAgent時發生錯誤!", vbExclamation, MsgTitle
            CreatePingAgent = False
            Exit Function
        End If
    Next
...
Private Function CreatePingCollector() As Boolean
    'log agent不能在Form Load時載入,否則此主程式尚未作用,收不到由log agent發來的訊息
    Dim ret As Long
    Dim fs As FileSystemObject
    Dim PingCollectorPath As String
    Dim appcount As Integer
    ...
    If Not fs.FileExists(PingCollectorPath) Then
        MsgBox "找不到PingCollector程式! (" & PingCollectorPath & ")", vbExclamation, MsgTitle
        Exit Function
    End If
    ...
    End If
    If ret = 0 Then
        MsgBox "執行CreatePingCollector時發生錯誤!", vbExclamation, MsgTitle
        Exit Function
    End If
...
Public Sub PingCollectorLoadPingListErr()
    picPingCollector.Picture = picLed(3).Picture '紅色
    picPingCollector.Refresh
End Sub
Private Function LoadListFromFile() As Boolean
    ...
    If Not fs.FileExists(glPingListFile) Then
        MsgBox glPingListFile & " 檔案不存在!", vbExclamation, MsgTitle
        Exit Function
    End If
    ...
    Exit Function
ErrHandler:
    MsgBox "由pinglist.txt載入資料時發生錯誤:" & vbCrLf & Err.Description, vbExclamation, MsgTitle
End Function
...
'    Exit Sub
'ErrHandler:
'    MsgBox "開啟log檔時發生以下錯誤:" & vbCrLf & Err.Description, vbExclamation, MsgTitle
'End Sub
...
Private Sub cmdOpenEventLog_Click()
    ...
    Set frm = New frmLog '此時已執行form Load event
...
Private Sub cmdStart_Click()
    ...
    '重新設定tick值,在啟動checkagent & checkcollector前
    'ReDim aryAgentBlinkTick(1 To glAgentCount)
...
''
    ...
    'ReDim aryAgentReady(1 To glAgentCount) '先宣告,以免在Ping!檢查時發生錯誤!
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
    ...
    Style = vbYesNo + vbCritical + vbDefaultButton1   ' 定義按鈕
    msg = "Are you sure you want to quit?"   ' 定義訊息
    response = MsgBox(msg, Style, title)
    If response = vbNo Then   ' 若使用者按下 [否]
        Cancel = True
        Exit Sub
    End If
...
'    Next
    ...
    tmrGetReportData.Enabled = False
    tmrChkAgentReady.Enabled = False
    UnHook '先unhook, 這樣就不會接收來至agent & collector的訊息了
    Call TellAgentToClose
    DoEvents
    Call TellCollectorToClose
    DoEvents
...
Private Sub PosAgentLed()
    ...
    Const ynum As Integer = 2 '最多顯示2列
...
'    Next
    ...
    xnum = (Picture1.Width - 150) / 285 '每個led的間隔為285
    If xnum > 0 Then
        k = 0
...
'        ColorGrid lvPingList
    ...
    AdjustColWidth lvPingList '要放在WM_SETREDRAW之後,否則圖示會有殘影
...
' 1 is the first column
    ...
    With lvDownList
        ...
        For i = 1 To .ColumnHeaders.Count
            ...
            Else
                DownListSortOrder(i) = lvwDescending '其它的設為Descending,下一次click時變為Ascending
            End If
        Next
...
' 1 is the first column
    ...
    With lvPingList
        ...
        For i = 1 To .ColumnHeaders.Count
            ...
            Else
                PingListSortOrder(i) = lvwDescending '其它的設為Descending,下一次click時變為Ascending
            End If
        Next
...
Private Sub lvPingList_DblClick()
    ...
    If lvPingList.SelectedItem Is Nothing Then
         ...
         Exit Sub
    End If
    Set itemx = lvPingList.SelectedItem '必須和MouseDown event同時作業
...
Private Sub ShowDownList()
    ...
    For i = 1 To lvPingList.ListItems.Count
        ...
        'If aryLastLedResult(i) = RESULT_DOWN Then '錯誤的方法,因為listview可能經過排序,和原來的list order不一樣了
...
Private Sub tmrChkAgentReady_Timer()
    ...
    '因為如果直接在pingagent keep alive reply將AgentReadyCount + 1, 會由於agent同時回應的關係,造成count失真
    Dim i As Integer
    ...
    If ChechAgentReadyCount >= 60 Then '每0.5秒檢查一次,超過30秒
        tmrChkAgentReady.Enabled = False
        ShowError "PingAgent逾時未回應Ready!"
    End If
End Sub
...
ErrHandler:
    Call StopLog
    MsgBox "儲存資料時產生下列錯誤:" & vbCrLf & Err.Description, vbExclamation, MsgTitle
End Sub
...
Public Sub CollectorLedBlink(BlinkSwitch As Long)
    ...
    Select Case BlinkSwitch
    Case MSG_PINGCOLLECTOR_BLINK_A '綠色
        picPingCollector.Picture = picLed(1).Picture
    Case MSG_PINGCOLLECTOR_BLINK_B '黃色
...
Private Sub tmrCheckAgentAlive_Timer()
    '檢查ping agent 和  ping collector是否都alive
    Dim curtick As Long
    Dim AgentID As Long
...
Public Sub AgentSayHello(AgentID As Long)
    ...
    TellAgentToLoadINI AgentID '--> agent回報
End Sub
...
Public Sub AgentReady(AgentID As Long)
    ...
    aryAgentBlinkTick(AgentID) = GetTickCount
    idx = AgentID - 1
    picAgent(idx).Picture = picLed(1).Picture 'Ready時改成綠色
    picAgent(idx).Refresh
    
    aryAgentReady(AgentID) = True
End Sub
Public Sub PingCollectorReady()
    '即PingCollector Load INI OK! --> 執行CreatePingAgent
    
    picPingCollector.Picture = picLed(1).Picture '綠色
    picPingCollector.Refresh
...
Public Sub AgentSayGoodbye(AgentID As Long)
    Dim idx As Long
    idx = AgentID - 1
    picAgent(idx).Picture = picLed(2).Picture 'red色
    picAgent(idx).Refresh
...
Public Sub PingCollectorSayGoodbye()
    picPingCollector.Picture = picLed(2).Picture 'Log Agent Say Hello時改成綠色 --> 變red色
    picPingCollector.Refresh
    glCollectorReady = False
    glPingCollectorHwnd = 0
End Sub
...
Private Sub tmrLog_Timer()
    ...
    If tdiff > glRefreshCycle Then
        '當TellConnectorSendEvent之後,collector必定會立刻回應,而且是在tmrLog的Interval之間
        '但是timer會有稍微誤差,故必須加上一容許值
        'MsgBox "curtick,lastlogtick,tdiff = " & curtick & " ," & LastLogTick & " ," & tdiff
        ShowError "log時間逾時 " & tdiff & "ms"
    End If
...
Public Sub PingCollectorSayHello()
    ...
    If glPingCollectorHwnd <> 0 Then
        picPingCollector.Picture = picLed(3).Picture '黃色
        picPingCollector.Refresh
...
Private Sub EnableCheckAgentReady()
    ...
    tmrChkAgentReady.Interval = 500 '每0.5秒檢查一次
