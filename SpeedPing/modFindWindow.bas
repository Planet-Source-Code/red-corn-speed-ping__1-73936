...
Private Function FindWindowPartial(ByVal titlepart As String, method As Integer) As Long
    ...
    hwndtmp = FindWindow(0&, 0&) '第二個參數If this parameter is NULL, all window names match
    Do Until hwndtmp = 0
        If hwndtmp <> glMyHwnd Then
            If GetParent(hwndtmp) = 0 Then
                titletmp = Space(256)
                ret = GetWindowText(hwndtmp, titletmp, Len(titletmp)) '傳回title字串的長度,不含terminating null character
                If ret <> 0 Then
